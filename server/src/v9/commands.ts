...
import { demoPdf, gateSubscriber, grantSubscriber, loadSubscribers } from "../v10/channel.js";
...
export async function dispatch(cmd: string, args: string[]): Promise<number> {
  ...
  if (cmd === "dossier") {
    if (flagRecord(args).demo === "true" || args.includes("--demo")) {
      console.log(JSON.stringify(demoPdf(), null, 2));
      return 0;
    }
    console.error("Live /dossier lookups run from the Telegram bot, or use intake. dossier --demo prints the fictional PDF. The bot never LOCKS.");
    return 0;
  }
  if (cmd === "channel") {
    return channelCommand(args);
  }
  return -1;
}

function channelCommand(args: string[]): number {
  const sub = args[0] || "list";
  const flags = flagRecord(args.slice(1));
  const id = Number(flags["telegram-id"] || flags.id || args[1]);
  if (sub === "list") {
    console.log(JSON.stringify(loadSubscribers(), null, 2));
    return 0;
  }
  if (!id) {
    console.error("Usage: channel grant --telegram-id 123 --days 30 | channel check --telegram-id 123");
    return 1;
  }
  if (sub === "grant") {
    const days = Number(flags.days || 30);
    console.log(JSON.stringify(grantSubscriber(id, days, "cli grant"), null, 2));
    return 0;
  }
  if (sub === "check") {
    console.log(JSON.stringify(gateSubscriber(id, flags.query || "Jane Doe"), null, 2));
    return 0;
  }
  console.error(`Unknown channel command: ${sub}`);
  return 1;
}
