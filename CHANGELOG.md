...
## v10.0.0 Channel (2026-09-24)

The Windows app stays free. A separate Telegram bot can send a public-source PDF to a paid member in a private chat.

- Members DM the bot. The membership room does not receive dossiers.
- `/dossier` never LOCKS. Every reply carries the investigative-lead line.
- Lookups of minors are refused.
- Access is an operator grant with a paid-through date. Default cap is 3 briefs a day.
- `spectra dossier --demo` prints the fictional Avery Quill PDF.
- The published Setup.exe is still the hashed unsigned 9.0.0 build. This version is the channel gate in source.
